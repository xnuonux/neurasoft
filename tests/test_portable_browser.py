...
def main():
    with sync_playwright() as p:
        ...
        page.set_content((ROOT.parent/'NEURASOFT_INTERACTIVE_PREVIEW.html').read_text(),wait_until='load')
        ...
        data=json.loads((ROOT/'private/portable-exhibit-export.json').read_text())
        ...
        browser.close()
    (ROOT/'private/portable-browser-report.json').write_text(json.dumps({'mode':'Offline browser, embedded site, local assets; not production deployment','passed':len(checks),'checks':checks},indent=2))
